...
public class Dummy_StepDefinition2 extends Base{
	...
	@BeforeClass
	public void setup() {
		fetchSession(Dummy_StepDefinition2.class);
		mydriver = LATEST_DRIVER_POOL.get(Dummy_StepDefinition2.class.getName());
		...
		ExtentTestManager.startTest(Dummy_StepDefinition2.class.getName());
		for (String url : urls) {
		...
		testURLS.put(Dummy_StepDefinition2.class.getName(), currentDomain);

		driverMap.put(Dummy_StepDefinition2.class.getName().split("\\.")[1], mydriver);
		pleaseWait(1, logger);
		logger.info("Browser pool at '" + Dummy_StepDefinition2.class.getName() + "' =>\n" + driverMap);
