...
public class SliderCarousel_StepDefinition extends SliderCarousel_page{
	...
	public void setup() {
		...
		mydriver.manage().timeouts().pageLoadTimeout(120, TimeUnit.SECONDS);if (fetchUrl("slider-carousel") == null) {
			if (Environment.equalsIgnoreCase("stage")) {
				urls.add("http://apsrs5642:8080/content/AutomationDirectory/about-us.html");
			} else if (Environment.equalsIgnoreCase("test")) {
				urls.add("http://apvrt31468:4503/content/AutomationDirectory/about-us.html");
			}
		} else {
			String[] scannedUrls = fetchUrl("slider-carousel").split(",");
		...
		setTagForTestClass("SliderCarousel", author, currentDomain, SliderCarousel_StepDefinition.class.getName());
...
//		mydriver.manage().deleteAllCookies();
	...
	public void sliderFunctionalityCheck() {
		skipNonExistingComponent(urls);
		for (String url : urls) {
	...
	public void conditionalSliderButtonAvailabilityCheck() {
		skipNonExistingComponent(urls);
		for (String url : urls) {
	...
	public void mainButtonAvailabilityCheck() {
		skipNonExistingComponent(urls);
		for (String url : urls) {
	...
	public void mainButtonBlankLabelCheck() {
		skipNonExistingComponent(urls);
		for (String url : urls) {
	...
	public void mainButtonRedirectionCheck() {
		skipNonExistingComponent(urls);
		for (String url : urls) {
